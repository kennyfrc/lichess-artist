        ...
        with open(self.outfn, 'a+') as f:
            ...
            if side:
                if self.passedPawnIsGood:
                    f.write('%d. %s {[%%eval %0.2f], %s} ' % (moveNumber, sanMove,
                            posScore, 'with a better passer'))
                    self.whitePassedPawnCommentCnt += 1
                elif self.kingSafetyIsGood:
                    f.write('%d. %s {[%%eval %0.2f], with a better king safety} ' %(
                            moveNumber, sanMove, posScore))
                    self.whiteKingSafetyCommentCnt += 1
                elif self.mobilityIsGood:
                    f.write('%d. %s {[%%eval %0.2f], with a better piece mobility} ' %(
                            moveNumber, sanMove, posScore))
                    self.whiteMobilityCommentCnt += 1
                elif self.matIsSacrificed:
                    f.write('%d. %s {[%%eval %0.2f], black had sacrificed material} ' %(
                            moveNumber, sanMove, posScore))
                else:
                    f.write('%d. %s {[%%eval %0.2f]} ' %(moveNumber, sanMove, posScore))
            else:
                if self.passedPawnIsGood:
                    f.write('%d... %s {[%%eval %0.2f], %s} ' % (moveNumber, sanMove,
                            posScore, 'with a better passer'))
                    ...
                    f.write('%d... %s {[%%eval %0.2f], white had sacrificed material} ' %(
                            moveNumber, sanMove, posScore))
                else:
                    f.write('%d... %s {[%%eval %0.2f]} ' % (moveNumber, sanMove, posScore))
            ...
            if side:
                if sanMove != engMove:
                    ...
                    if self.matIsSacrificed:
                        if moveNag == '$0':
                            if varComment == '':
                                f.write('%d. %s {[%%eval %0.2f], black had sacrificed material} (%s {[%%eval %0.2f]}) ' % (
                                    moveNumber, sanMove, posScore, pvLine, engScore))
                            else:
                                f.write('%d. %s {[%%eval %0.2f], black had sacrificed material} ({%s} %s {[%%eval %0.2f]}) ' % (
                                    moveNumber, sanMove, posScore, varComment,
                                    pvLine, engScore))
                        else:
                            if varComment == '':
                                f.write('%d. %s %s {[%%eval %0.2f], black had sacrificed material} (%s {[%%eval %0.2f]}) ' % (
                                    moveNumber, sanMove, moveNag, posScore,
                                    pvLine, engScore))
                            else:
                                f.write('%d. %s %s {[%%eval %0.2f], black had sacrificed material} ({%s} %s {[%%eval %0.2f]}) ' % (
                                    moveNumber, sanMove, moveNag, posScore,
                                    varComment, pvLine, engScore))
                    else:  
                        if moveNag == '$0':
                            if varComment == '':
                                f.write('%d. %s {[%%eval %0.2f]} (%s {[%%eval %0.2f]}) ' % (
                                    moveNumber, sanMove, posScore, pvLine, engScore))
                            else:
                                f.write('%d. %s {[%%eval %0.2f]} ({%s} %s {[%%eval %0.2f]}) ' % (
                                    moveNumber, sanMove, posScore, varComment,
                                    pvLine, engScore))
                        else:
                            if varComment == '':
                                f.write('%d. %s %s {[%%eval %0.2f]} (%s {[%%eval %0.2f]}) ' % (
                                    moveNumber, sanMove, moveNag, posScore,
                                    pvLine, engScore))
                            else:
                                f.write('%d. %s %s {[%%eval %0.2f]} ({%s} %s {[%%eval %0.2f]}) ' % (
                                    moveNumber, sanMove, moveNag, posScore,
                                    varComment, pvLine, engScore))
                # Else if game and engine move are the same
                    ...
                    if threatMove is None:
                        if self.passedPawnIsGood:
                            self.whitePassedPawnCommentCnt += 1
                            if moveNag == '$0':
                                f.write('%d. %s {[%%eval %0.2f], with a better passer} ' % (
                                        moveNumber, sanMove, posScore))
                            else:
                                f.write('%d. %s %s {[%%eval %0.2f], with a better passer} ' % (
                                        moveNumber, sanMove, moveNag, posScore))
                        elif self.kingSafetyIsGood:
                            self.whiteKingSafetyCommentCnt += 1
                            if moveNag == '$0':
                                ...
                                f.write('%d. %s %s {[%%eval %0.2f], with a better king safety} ' % (
                                        moveNumber, sanMove, moveNag, posScore))
                        elif self.mobilityIsGood:
                            self.whiteMobilityCommentCnt += 1
                            if moveNag == '$0':
                                ...
                                f.write('%d. %s %s {[%%eval %0.2f], with a better piece mobility} ' % (
                                        moveNumber, sanMove, moveNag, posScore))
                        elif self.matIsSacrificed:
                            if moveNag == '$0':
                                f.write('%d. %s {[%%eval %0.2f], black had sacrificed material} ' % (
                                ...
                                f.write('%d. %s %s {[%%eval %0.2f], black had sacrificed material} ' % (
                                        moveNumber, sanMove, moveNag, posScore))
                        else:
                            if moveNag == '$0':
                                f.write('%d. %s {[%%eval %0.2f]} ' %(
                                        moveNumber, sanMove, posScore))
                            else:
                                f.write('%d. %s %s {[%%eval %0.2f]} ' % (
                                    moveNumber, sanMove, moveNag, posScore))
                    # Else if there is threat move
                    else:
                        if moveNag == '$0':
                            f.write('%d. %s {[%%eval %0.2f], %s %s} ' % (
                                moveNumber, sanMove, posScore,
                                sr.choice(PLAN_COMMENT), threatMove))
                        else:
                            f.write('%d. %s %s {[%%eval %0.2f], %s %s} ' % (
                                moveNumber, sanMove, moveNag, posScore,
                                sr.choice(PLAN_COMMENT), threatMove))
            # Else if side to move is black
                ...
                if sanMove != engMove:
                    ...
                    if self.matIsSacrificed:
                        if moveNag == '$0':
                            if varComment == '':
                                f.write('%d... %s {[%%eval %0.2f], white had sacrificed material} (%s {[%%eval %0.2f]}) ' % (
                                    moveNumber, sanMove, posScore, pvLine,
                                    engScore))
                            else:
                                f.write('%d... %s {[%%eval %0.2f], white had sacrificed material} ({%s} %s {[%%eval %0.2f]}) ' % (
                                    moveNumber, sanMove, posScore, varComment,
                                    pvLine, engScore))
                        else:
                            if varComment == '':
                                f.write('%d... %s %s {[%%eval %0.2f], white had sacrificed material} (%s {[%%eval %0.2f]}) ' % (
                                    moveNumber, sanMove, moveNag, posScore,
                                    pvLine, engScore))
                            else:
                                f.write('%d... %s %s {[%%eval %0.2f], white had sacrificed material} ({%s} %s {[%%eval %0.2f]}) ' % (
                                    moveNumber, sanMove, moveNag, posScore,
                                    varComment, pvLine, engScore))
                    else:
                        if moveNag == '$0':
                    ...
                    if threatMove is None:
                        if self.passedPawnIsGood:
                            ...
                            if moveNag == '$0':
                                ...
                                f.write('%d... %s %s {[%%eval %0.2f], white had sacrificed material} ' % (
                                        moveNumber, sanMove, moveNag, posScore))
                        else:
                            if moveNag == '$0':
                                f.write('%d... %s {[%%eval %0.2f]} ' % (
        ...
        with open(self.outfn, 'a+') as f:
            ...
            if side:
                if sanMove != engMove:
                    ...
                    if moveNag == '$0':
                        if varComment == '':
                            f.write('%d. %s {[%%eval %0.2f]} (%d. %s {%s}) (%s {[%%eval %0.2f]}) ' % (
                                moveNumber, sanMove, posScore, moveNumber,
                                bookMove, BOOK_COMMENT, pvLine, engScore))
                        else:
                            f.write('%d. %s {[%%eval %0.2f]} (%d. %s {%s}) ({%s} %s {[%%eval %0.2f]}) ' % (
                                moveNumber, sanMove, posScore, moveNumber,
                                bookMove, BOOK_COMMENT, varComment, pvLine,
                                engScore))
                    else:
                        if varComment == '':
                            f.write('%d. %s %s {[%%eval %0.2f]} (%d. %s {%s}) (%s {[%%eval %0.2f]}) ' % (
                                moveNumber, sanMove, moveNag, posScore,
                                moveNumber, bookMove, BOOK_COMMENT, pvLine,
                                engScore))
                        else:
                            f.write('%d. %s %s {[%%eval %0.2f]} (%d. %s {%s}) ({%s} %s {[%%eval %0.2f]}) ' % (
                                moveNumber, sanMove, moveNag, posScore,
                ...
                # Else if game and engine move are the same
                    ...
                    if threatMove is not None:
                        if moveNag == '$0':
                            f.write('%d. %s {[%%eval %0.2f], %s %s} (%d. %s {%s}) ' % (
                                moveNumber, sanMove, posScore,
                                sr.choice(PLAN_COMMENT), threatMove,
                                moveNumber, bookMove, BOOK_COMMENT))
                        else:
                            f.write('%d. %s %s {[%%eval %0.2f], %s %s} (%d. %s {%s}) ' % (
                                moveNumber, sanMove, moveNag, posScore,
                                sr.choice(PLAN_COMMENT), threatMove,
                                moveNumber, bookMove, BOOK_COMMENT))
